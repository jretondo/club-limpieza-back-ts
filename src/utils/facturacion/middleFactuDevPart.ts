...
const factuMiddelDevPart = () => {
  const middleware = async (
    ...
    try {
      ...
      porcPago =
        (productsList.totalFact - montoCtaCte) / productsList.totalFact;
      costo = productsList.totalCosto * porcPago;
      costoImputar = productsList.totalCosto - costo;

      console.log('pvData[0].cond_iva :>> ', pvData[0].cond_iva);
      console.log('pvdata cond iva type :>> ', typeof pvData[0].cond_iva);
